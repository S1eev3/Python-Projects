...
def first_run(self):
    ...
    with conn:
        ...
        if count < 1:
            cur.execute("""INSERT INTO tbl_phonebook (col_fname,col_lname,col_fullname,col_phone,col_email) VALUES (?,?,?,?,?)""", ('John','Doe','John Doe','111-111-1111','jdoegmail.com'))
            conn.commit()
    conn.close()
...
def addToList(self):
    ...
    if (len(var_fname) > 0) and (len(var_lname) > 0) and (len(var_phone) > 0) and (len(var_email) > 0): # enforce user to enter input in fields
        conn = sqlite3.connect('phonebook.db')
        with conn:
            cursor = conn.cursor()
