...
def center_window(self, w, h): # Pass in tkinter master frame reference and w and h
    # get users screen width and height
    screen_width = self.master.winfo_screenwidth()
    screen_height = self.master.winfo_screenheight()
    # calculates x and y coordinates to paint the app centered on the suers screen
...
def create_db(self):
    ...
    with conn:
        ...
        # commiting to save changes and close dB connection
        conn.commit()
    conn.close()
    first_run(self)
...
def first_run(self):
    ...
    with conn:
        ...
        if count < 1:
            cur.execute("""INSERT INTO tbl_students (col_fname,col_lname,col_fullname,col_phone,col_email,col_course) VALUES (?,?,?,?,?,?)""", ('John','Doe','John Doe','111-111-1111','jdoegmail.com', 'Full-Stack'))
            conn.commit()
    conn.close()
...
def addToList(self):
    ...
    var_fullname = ("{} {}".format(var_fname,var_lname)) #combine normailzed names into a fullname
    ...
    if (len(var_fname) > 0) and (len(var_lname) > 0) and (len(var_phone) > 0) and (len(var_email) > 0) and (len(var_course) > 0): # enforce user to enter input in fields
        conn = sqlite3.connect('students.db')
        with conn:
            cursor = conn.cursor()
            #Check the dB for the existence of the fullname, if so we will alert the user and disregard the request
...
def onDelete(self):
    ...
    with conn:
        ...
        # th dB...cannot delete the last record or we will get an error
        cur.execute("""SELECT COUNT(*) FROM tbl_students""")
        count = cur.fetchone()[0]
        if count > 1:
            ...
            if confirm:
                ...
                onDeleted(self) # call the funtion to clear all of the textboxes and the selected index of listbox
...
def onUpdate(self):
    ...
    # For name changes, the user will need to delete the entire recoed and start over.
